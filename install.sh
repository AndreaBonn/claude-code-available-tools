...
SLASH_CMD_SRC="$SCRIPT_DIR/slash-command/tools.md"
...
if command -v pipx &>/dev/null; then
    info "Using pipx..."
    pipx install "$SCRIPT_DIR" --force 2>&1 | tail -5
    ok "Installed via pipx"
elif command -v uv &>/dev/null; then
    info "Using uv..."
    uv tool install "$SCRIPT_DIR" --force 2>&1 | tail -5
    ok "Installed via uv tool"
else
    info "Using pip install --user..."
    python3 -m pip install --user "$SCRIPT_DIR" 2>&1 | tail -5
